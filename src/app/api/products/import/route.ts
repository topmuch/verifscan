...
import { parseFileToRows, importProducts } from "@/lib/import";

/**
 * POST /api/products/import
 *
 * Multipart form-data with field "file" = CSV or XLSX file.
 * Imports multiple products in one shot.
 *
 * Required columns (case-insensitive, French or English accepted):
 *   - name | nom | produit
 *   - brand | marque
 *   - category | categorie | catégorie
 *   - (optional) description, weight, barcode, variety, region, gpsLat, gpsLng, photoUrl...
 *
 * Returns:
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const summary = await importProducts(rows, user.id);
